...
class AddRemoveFoldersTest(TestBase):
    def run(self):
        try:
            ...
            self.srv.syncthingHandler.addFolder(fpath0s, 'folder_A', [self.cl0.syncthingHandler.myId()]).result()
            self.srv.syncthingHandler.addFolder(fpath1s, 'floder_B', [self.cl1.syncthingHandler.myId()]).result()
            self.srv.syncthingHandler.addFolder(fpath1s, 'floder_C', [self.cl1.syncthingHandler.myId()]).result()
