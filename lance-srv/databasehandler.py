...
class DatabaseHandler(ServerComponent):
	...
	def get_configuration(self):
		...
		try:
			...
			for fid, flabel, fpath in folders:
				...
				cfg['folders'][fid] = {'label': flabel, 'path': os.path.normpath(fpath), 'type': 'sendreceive', 'devices': []}
				for _, _, dev in acs:
	...
	def get_devices(self):
		...
		try:
			cur = con.cursor()
			devices = cur.execute("SELECT device,userid FROM devices INNER JOIN users ON devices.userid=users.userid WHERE devices.blocked=0 AND users.access<>0").fetchall()
		finally:
			con.close()
